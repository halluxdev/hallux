...
from issues.file_diff import DiffProposal
...
def test_file_diff_parametrized(
    ...
    filediff = DiffProposal(str(test_file), issue_line=issue_line, radius_or_range=radius)
...
def test_file_diff(test_filename="file_diff_test.txt"):
    test_file = Path(__file__).resolve().parent.joinpath(test_filename)
    try:
        DiffProposal(str(test_file), issue_line=100)
        pytest.fail("FileDiff shall raise an Exception")
    except Exception:
        pass  # Exception intercepted!

    fd0 = DiffProposal(str(test_file), issue_line=4, radius_or_range=0)
    assert fd0.issue_lines == ["4"]

    fd1 = DiffProposal(str(test_file), issue_line=4, radius_or_range=1)
    assert fd1.issue_lines == ["3", "4", "5"]

    fd2 = DiffProposal(str(test_file), issue_line=4, radius_or_range=2)
    assert fd2.issue_lines == ["2", "3", "4", "5", "6"]

    fd3 = DiffProposal(str(test_file), issue_line=4, radius_or_range=3)
    ...
    fd1_c = DiffProposal(str(test_file), issue_line=4, radius_or_range=1, issue_line_comment=" #")
...
def test_cpp_file_diff_with_real_openai_results(test_filename="../test-cpp-project/cpp/test_cpp_project.cpp"):
    test_file = Path(__file__).resolve().parent.joinpath(test_filename)
    fd = DiffProposal(str(test_file), issue_line=21, radius_or_range=4, issue_line_comment=" // line 21")
    ...
    fd2 = DiffProposal(str(test_file), issue_line=21, radius_or_range=5, issue_line_comment=" // line 21")
...
def test_python_file_diff_with_real_openai_results(
    ...
    test_file = Path(__file__).resolve().parent.joinpath(test_filename)
    fd = DiffProposal(str(test_file), issue_line=39, radius_or_range=4)
