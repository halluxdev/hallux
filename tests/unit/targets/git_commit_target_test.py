...
def test_commit_diff(git_target_factory, diff_proposal):
    with patch('subprocess.check_output') as mock_subprocess, \
        ...
        # Assertions
        assert git_target.commit_diff() == True
...
def test_commit_diff_failure(git_target_factory, diff_proposal):
    with patch.object(FilesystemTarget, 'revert_diff', return_value=None) as mock_fs_revert_diff, \
            patch('os.getcwd', return_value=TEST_PROJECT_PATH), \
            patch('os.chdir'):
        ...
        assert git_target.commit_diff() == False
...
def test_commit_diff_failure_verbose(git_target_factory, diff_proposal):
    with patch.object(FilesystemTarget, 'revert_diff', return_value=None), \
            patch('os.getcwd', return_value=TEST_PROJECT_PATH), \
            patch('os.chdir'):
